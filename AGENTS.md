...
- Developers add and update localization keys manually only in `LocalPackages/TKLocalize/Sources/TKLocalize/Resources/Locales/en.lproj/Localizable.strings`.
- Agents generate the other locale files in `LocalPackages/TKLocalize/Sources/TKLocalize/Resources/Locales/*.lproj/Localizable.strings` based on the English source file.
- `LocalPackages/TKLocalize/Sources/TKLocalize/TKLocales.swift` must never be edited by hand by either developers or agents.
- Regenerate `LocalPackages/TKLocalize/Sources/TKLocalize/TKLocales.swift` only via `make locale` from the repository root.

## Analytics Workflow
- Run analytics generation only via `make analytics` from the repository root.
- `make analytics` runs `scripts/analytics/sync_models.sh`, which updates the analytics schemas checkout at `.context/analytics-schemas` by default. Override the source checkout with `ANALYTICS_SCHEMAS_ROOT` when needed.
- The whitelist lives in `scripts/analytics/event_model_whitelist.txt`; it is sorted automatically by the sync script on every run.
- Whitelisted models that still exist in the current schema are copied into `LocalPackages/TKCore/Sources/TKCore/Analytics/Events/Generated`.
- Whitelisted models that were removed from the current schema are preserved as backports in `LocalPackages/TKCore/Sources/TKCore/Analytics/Events/Deprecated`. On the first run after removal, the sync script moves the existing generated file into `Deprecated`; later runs keep using that deprecated copy until the event is removed from the whitelist.
- Files that are no longer present in the whitelist are deleted from both `Generated` and `Deprecated`.
