    ...
    func decryptMnemonics(_ encryptedMnemonics: EncryptedMnemonics, password: String) async throws -> Mnemonics {
        ...
        for (identifier, item) in decrypted {
            do {
                ...
                dropped += 1
                logger.e(
                    "🪵 rn mnemonics decode dropped invalid item id=\(identifier.redactedMnemonicIdentifier): \(error)"
                )
            }
        }
        if dropped > 0 {
            logger.e("🪵 rn mnemonics decode summary: total=\(decrypted.count), dropped=\(dropped)")
        }
        return result
    }
