...
public struct TronUSDTAPI {
    ...
    public func hasTransactions(address: Address) async throws -> Bool {
        let response = try await tronApi.getTronHistory(
            address: address,
            limit: 1,
            minTimestamp: nil,
            maxTimestamp: nil,
    ...
    public func estimateBatteryCharges(address: Address, method: ContractMethod) async throws -> (energy: Int, bandwidth: Int, estimateCharges: Int) {
        ...
        return (marginEnergy, effectiveBandwidth, estimate.total_charges)
    }
