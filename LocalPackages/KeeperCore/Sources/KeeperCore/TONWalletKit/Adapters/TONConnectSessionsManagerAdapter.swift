...
final class TONConnectSessionsManagerAdapter: TONConnectSessionsManager {
    ...
    private func sessions() async throws -> [TONConnectSession] {
        ...
        for wallet in wallets {
            ...
            do {
                apps = try tonConnectService.getConnectedApps(forWallet: wallet)
            } catch {
                Log.e(
                    "\(String(reflecting: Self.self)): failed to fetch connected apps for wallet in sessions()",
                    extraInfo: [
            ...
            for app in apps.apps {
                do {
                    let session = try buildSession(from: app, wallet: wallet)
                    allSessions.append(session)
                } catch {
                    Log.e(
                        "\(String(reflecting: Self.self)): failed to build session for connected app in sessions()",
                        extraInfo: [
                            "error": error.localizedDescription,
                            "walletId": wallet.id,
    ...
    func removeSession(id: TONConnectSessionID) async throws {
        for wallet in walletsStore.wallets {
            ...
            do {
                ...
                Log.e(
                    "\(String(reflecting: Self.self)): failed to fetch connected apps for wallet in removeSession(id:)",
                    extraInfo: [
                        "error": error.localizedDescription,
                        "walletId": wallet.id,
                        "sessionId": id,
    ...
    func removeSessions(filter: TONConnectSessionsFilter?) async throws {
        ...
        for session in sessions {
            do {
                ...
                Log.e(
                    "\(String(reflecting: Self.self)): failed to remove session in removeSessions(filter:)",
                    extraInfo: [
                        "error": error.localizedDescription,
    ...
    func removeAllSessions() async throws {
        for wallet in walletsStore.wallets {
            ...
            do {
                ...
                Log.e(
                    "\(String(reflecting: Self.self)): failed to fetch connected apps for wallet in removeAllSessions()",
                    extraInfo: [
                        "error": error.localizedDescription,
                        "walletId": wallet.id,
                    ]
            ...
            for app in apps.apps {
                do {
                    ...
                    Log.e(
                        "\(String(reflecting: Self.self)): failed to disconnect app in removeAllSessions()",
                        extraInfo: [
                            "error": error.localizedDescription,
                            "walletId": wallet.id,
                            "appUrl": app.manifest.url.absoluteString,
                        ]
                    )
                }
            }
