...
public protocol QRCodeGenerator {
    func generate(string: String, size: CGSize, cgImageBacked: Bool) async -> UIImage?
}

public extension QRCodeGenerator {
    func generate(string: String, size: CGSize) async -> UIImage? {
        await generate(string: string, size: size, cgImageBacked: false)
    }
}
