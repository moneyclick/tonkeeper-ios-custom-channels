...
extension UINavigationController: @retroactive UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        canBeginSwipeBackGesture(gestureRecognizer)
    }
}
...
extension UINavigationController {
    func canBeginSwipeBackGesture(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        guard let panGestureRecognizer = gestureRecognizer as? UIPanGestureRecognizer else {
            return false
        }

        let velocity = panGestureRecognizer.velocity(in: view)
        let isRightSwipe = velocity.x > 0 && abs(velocity.x) > abs(velocity.y)
        let isSystemSwipeToBackEnabled = interactivePopGestureRecognizer?.isEnabled == true
        let isThereStackedViewControllers = viewControllers.count > 1
        // Starting an interactive pop while UIKit is still dismissing a modal
        // can leave presentation overlays attached.
        let isNavigationTransitioning = transitionCoordinator != nil
        let hasPresentedViewController =
            presentedViewController != nil
                || topViewController?.presentedViewController != nil

        return isRightSwipe
            && isSystemSwipeToBackEnabled
