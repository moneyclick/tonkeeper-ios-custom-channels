...
final class NativeSwapTransactionConfirmationContainerView: UIView {
    ...
    private func setupConstraints() {
        ...
        feeView.snp.makeConstraints { make in
            make.top.equalTo(valueDifference.snp.bottom)
            make.left.right.equalTo(receiveContainerView)
            make.height.equalTo(36)
        }
    ...
    private func didUpdateConfiguration() {
        ...
        if let valueDifferenceItem = configuration.valueDifference {
            valueDifference.isHidden = false
            valueDifference.snp.remakeConstraints { make in
                make.top.equalTo(slippageView.snp.bottom)
                make.left.right.equalTo(receiveContainerView)
                make.height.equalTo(36)
            }
            feeView.snp.remakeConstraints { make in
                make.top.equalTo(valueDifference.snp.bottom)
                make.left.right.equalTo(receiveContainerView)
                make.height.equalTo(36)
            }
            valueDifference.update(
                title: valueDifferenceItem.title,
                value: valueDifferenceItem.value,
                valueColor: valueDifferenceItem.valueColor,
                valueIcon: valueDifferenceItem.valueIcon,
                captionModel: .init(
                    title: nil,
                    icon: TKPlainButton.Model.Icon(
                        image: .TKUIKit.Icons.Size16.informationCircle,
