...
final class NativeSwapTransactionConfirmationCoordinator: RouterCoordinator<NavigationControllerRouter> {
    ...
    func openConfirmation() {
        ...
        module.output.didProduceTonSwapInsufficientFeeError = { [weak self] requiredFee, balance in
            self?.showTonSwapInsufficientFeePopup(
    ...
    func showTonSwapInsufficientFeePopup(
        requiredFee: BigUInt,
        balance: BigUInt
    ) {
        let tonToken = TonToken.ton
        let amountFormatter = keeperCoreMainAssembly.formattersAssembly.amountFormatter
        let requiredFeeFormatted = amountFormatter.format(
            amount: requiredFee,
            fractionDigits: tonToken.fractionDigits
        )
        let balanceFormatted = amountFormatter.format(
        ...
        buyButtonConfiguration.content = TKButton.Configuration.Content(
            title: .plainString(TKLocales.InsufficientFunds.buyTokenTitle(tonToken.symbol))
        )
        buyButtonConfiguration.action = { [weak self] in
            self?.didRequestOpenBuySell?(true)
        }
        ...
        viewController.configuration = InfoPopupBottomSheetViewController.Configuration(
            ...
            title: TKLocales.TronUsdtFees.Common.FeeOptions.Title.insufficient,
            caption: TKLocales.InsufficientFunds.feeRequired(
                requiredFeeFormatted,
                balanceFormatted
            ),
            bodyContent: nil,
            buttons: [buyButtonConfiguration]
