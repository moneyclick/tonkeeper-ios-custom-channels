...
final class SendV3ViewModelImplementation: SendV3ViewModel, SendV3ModuleOutput, SendV3ModuleInput {
    ...
    func viewDidLoad() {
        ...
        case let .withdraw(_, exchangeTo):
            ...
            let mainPart = "\(TKLocales.Receive.title(exchangeTo.symbol)) ".withTextStyle(.h3, color: .Text.primary)
    ...
    private func fiatInputString(amount: BigUInt, convert: () -> String) -> String {
        guard amount > 0 else { return "" }
        return sendAmountTextFieldFormatter.unformatString(convert()) ?? ""
    }
                ...
                if isSwapped {
                    if let lastFiatInputString {
                        ...
                        formatted = fiatInputString(amount: amount) {
                            sendController.convertTokenAmountToCurrency(token: token, amount, false)
                        }
                    }
    ...
    private func updateViewState() {
        ...
        let commentState: SendV3ViewModelViewState.CommentState? = {
            if case .withdraw = sendInput { return nil }
    ...
    private func limitErrorForCurrentAmount() -> String? {
        ...
        if let min = limits.min, amountDouble < min {
            return TKLocales.Ramp.Withdraw.minAmount("\(min)")
        }
