...
final class HistoryEventDetailsMapper {
    ...
    func mapJettonTransfer(
        ...
        let title = signedAmountFormatter.format(
            ...
            accessory: action.jettonInfo.symbol.flatMap { .tokenSymbol($0) } ?? .none,
            isNegative: amountType == .outcome,
            style: .exactValue
        )
        ...
        let management: HistoryEventDetailsModel.Management? = {
            ...
            let isManagementAvailable: Bool = {
                ...
                guard let rate = balance.rates.first(where: { $0.key == .GRAM })?.value else {
                    return false
                }
    ...
    private func mapPurchaseAction(
        ...
        let title = signedAmountFormatter.format(
            ...
            accessory: .tokenSymbol(action.amount.tokenName),
            isNegative: true,
            style: .exactValue
        )
    ...
    func mapWithdrawStakeRequest(
        ...
        if let stakeMeta = action.stakeMeta {
            let formattedAmount = signedAmountFormatter.format(
                amount: BigUInt(stringLiteral: stakeMeta.value),
                fractionDigits: stakeMeta.decimals,
                accessory: .tokenSymbol(stakeMeta.tokenName),
                isNegative: true
            )
        ...
        } else if let amount = action.amount {
            let formattedAmount = signedAmountFormatter.format(
