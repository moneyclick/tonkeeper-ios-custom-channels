...
final class HistoryListViewModelImplementation: HistoryListViewModel, HistoryListModuleOutput, HistoryListModuleInput {
    ...
    private func didGetHistoryLoaderEvent(_ event: HistoryPaginationLoader.Event) {
        queue.async { [weak self] in
            ...
            case let .pageLoaded(events, hasMore):
                let filteredEventsPage = uniqueEvents(events, excluding: Set(self.events.map(\.eventId)))
                self.events += filteredEventsPage
                let transactionsManagementState = transactionsManagementStore.state
                let filteredEvents = filterEvents(filteredEventsPage, filter: _filter, transactionsManagementState: transactionsManagementState)
    ...
    private func uniqueEvents(
        ...
        excluding existingEventIDs: Set<AccountEvent.EventID> = []
    ) -> [HistoryEvent] {
        var seenEventIDs = existingEventIDs
        return events.compactMap { event in
            guard seenEventIDs.insert(event.eventId).inserted else { return nil }
            return event
        }
    }
    ...
    private func createSnapshot(listState: State) -> HistoryList.Snapshot {
        ...
        case let .content(content, pagination):
            if content.isEmpty {
                ...
                var sectionIds = Set<HistoryList.SnapshotSection>()
                var eventIds = Set<HistoryList.EventID>()
                for section in content.sections {
                    let sectionId = HistoryList.SnapshotSection.events(section.date)
                    let sectionItems = HistoryList.uniqueEventSnapshotItems(
                        identifiers: section.events.lazy.map(\.identifier),
                        seenIdentifiers: &eventIds
                    )
                    guard !sectionItems.isEmpty else { continue }
                    if sectionIds.insert(sectionId).inserted {
