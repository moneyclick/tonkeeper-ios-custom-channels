...
final class WalletContainerTopBarView: UIView, ConfigurableView {
    ...
    private let historyButton = TKButton(frame: .zero)
    private let settingsButton = TKIndicatorButton(frame: .zero)
    private lazy var trailingButtonsStackView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [historyButton, settingsButton])
    ...
    var historyButtonTooltipSourceView: UIView? {
        historyButton.isHidden ? nil : historyButton
    }
    ...
    func configure(model: Model) {
        ...
        if let historyButtonConfiguration = model.historyButtonConfiguration {
            historyButton.configuration = historyButtonConfiguration
        }
        setHistoryButtonVisibility(model.historyButtonConfiguration != nil)
...
private extension WalletContainerTopBarView {
    func setup() {
        ...
        contentContainerView.addSubview(trailingButtonsStackView)
    ...
    func setupConstraints() {
        ...
        historyButton.setContentCompressionResistancePriority(.required, for: .horizontal)
        ...
        walletButton.snp.makeConstraints { make in
            ...
            make.right.lessThanOrEqualTo(trailingButtonsStackView.snp.left)
        }
        ...
        trailingButtonsStackView.snp.makeConstraints { make in
        ...
        setHistoryButtonVisibility(false)
    }

    func setHistoryButtonVisibility(_ isVisible: Bool) {
        historyButton.isHidden = !isVisible
    }
}
