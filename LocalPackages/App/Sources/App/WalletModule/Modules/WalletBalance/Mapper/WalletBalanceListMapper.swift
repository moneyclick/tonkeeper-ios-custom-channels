...
struct WalletBalanceListMapper {
    ...
    func mapTonItem(
        ...
        return WalletBalanceListCell.Configuration(
            walletBalanceListCellContentViewConfiguration: WalletBalanceListCellContentView.Configuration(
                listItemContentViewConfiguration: balanceItemMapper.mapTonItem(
                    item,
                    tags: tonTags(for: wallet),
                    isSecure: isSecure,
                    isPinned: isPinned
                ),
    ...
    private func tonTags(for wallet: Wallet) -> [TKTagView.Configuration] {
        guard
            let text = tonStakingAPYTextFormatter(tonStakingAPYProvider(wallet))
        else {
            return []
        }

        return [
            .accentTag(text: text, color: .Accent.green),
        ]
    }
}
