#!/usr/bin/env python3
"""Local runner for the tk-impact-analysis skill."""
...
def render_selected_regress_rows(recommendations: list[tuple[str, RegressEntry]], regress_entries: list[RegressEntry]) -> list[str]:
    ...
    for entry in regress_entries:
        if entry.line_no in selected_line_nos:
            included_section_line_nos.update(
                parent_line_no
                for parent_line_no in entry.parent_line_nos
                if entries_by_line_no[parent_line_no].is_section
            )
