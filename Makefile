...
trading_generate:
	@scripts/trading_apigen/generate_api.sh

multichain_generate:
	@scripts/multichain_apigen/generate_api.sh
...
compile:
	...
	mkdir -p $(BUILD_DIR) $(BUILD_DIR)/codex_home
	@if command -v gh >/dev/null 2>&1; then \
		TOKEN=$$(gh auth token 2>/dev/null || true); \
		if [ -n "$$TOKEN" ]; then \
			umask 077; \
			{ \
				for host in github.com api.github.com codeload.github.com objects.githubusercontent.com release-assets.githubusercontent.com; do \
					printf '%s\n' "machine $$host"; \
					printf '%s\n' "login x-access-token"; \
					printf 'password %s\n\n' "$$TOKEN"; \
				done; \
			} > $(BUILD_DIR)/codex_home/.netrc; \
		fi; \
	fi
...
test_all: test_core_swift test_tron_swift_package test_tkcore_package test_tklocalize_package test_tkchart_package test_app_package
